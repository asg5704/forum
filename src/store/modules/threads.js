...
export default {
  ...
  getters: {
    threadRepliesCount: state => id => countObjectProps(state.items[id].posts) - 1
  ...
  actions: {
    createThread: ({commit, state, dispatch, rootState}, {text, title, forumId}) => {
      return new Promise((resolve, reject) => {
        ...
        .then(() => {
          // updates the threads
          commit('setItem', {resource: 'threads', item: thread, itemId: threadId}, {root: true})
    ...
    updateThread: ({commit, state, dispatch, rootState}, {text, title, id}) => {
      return new Promise((resolve, reject) => {
